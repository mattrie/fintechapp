...
//           balance > 1 is to avoid repetition
     $result = mysqli_query($connect, "SELECT * FROM allloan WHERE  year = '$year' AND (insurance_in > '1' OR insurance_out) ORDER BY id DESC");  
     $result11 = mysqli_query($connect, "SELECT * FROM weekly_allloan WHERE  year = '$year' AND (insurance_in > '1' OR insurance_out) ORDER BY id DESC");  
      $result22 = mysqli_query($connect, "SELECT * FROM monthly_allloan WHERE  year = '$year' AND (insurance_in > '1' OR insurance_out) ORDER BY id DESC");  
    
    echo '<div class="card card-block table-border-style">';
     
     echo     '<div class="table-responsive">
     <CAPTION><h3 style="font-size:24px; color:black; font-weight:bold;" align="center">INSURANCE LEDGER ['.$year.']</h3></CAPTION>
  ...
  <table class="table table-bordered table-striped table-sm  table-hover "  align="center">
    ...
    <tbody class="" style="font-family: sans-serif; color:black ; font-weight:bold;">';
         ...
         $sum_insurance_in = mysqli_query($connect, "SELECT SUM(insurance_in) as total_in FROM allloan WHERE  year = '$year'");
                   while ($row = mysqli_fetch_assoc($sum_insurance_in)) {
                        $insurance_in = $row['total_in'];
                   }


                   $sum_insurance_out = mysqli_query($connect, "SELECT SUM(insurance_out) as total_out FROM allloan WHERE  year = '$year'");
         ...
         $sum_insurance_in2 = mysqli_query($connect, "SELECT SUM(insurance_in) as total_in FROM weekly_allloan WHERE  year = '$year'");
                   while ($row = mysqli_fetch_assoc($sum_insurance_in2)) {
                        $insurance_in2 = $row['total_in'];
                   }


                   $sum_insurance_out2 = mysqli_query($connect, "SELECT SUM(insurance_out) as total_out FROM weekly_allloan WHERE  year = '$year'");
         ...
         $sum_insurance_in3 = mysqli_query($connect, "SELECT SUM(insurance_in) as total_in FROM monthly_allloan WHERE  year = '$year'");
                   while ($row = mysqli_fetch_assoc($sum_insurance_in3)) {
                        $insurance_in3 = $row['total_in'];
                   }


                   $sum_insurance_out3 = mysqli_query($connect, "SELECT SUM(insurance_out) as total_out FROM monthly_allloan WHERE  year = '$year'");
