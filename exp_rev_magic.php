...
<html>
    ...
    <body style="background-color: #e0d3d3;">
     ...
     $result = mysqli_query($connect, "SELECT * FROM revenuexp ORDER BY id DESC LIMIT 1000");
        echo ' <div class="card">';
         echo '<div class="card-block table-border-style">';
     
     echo     '<div class="table-responsive" >';              
     echo '<CAPTION><h3 align="center" style="font-size:22px; color: black;">ALL REVENUE & EXPENDITURE</h3></CAPTION>';
    ...
    echo '<tbody class="" style="font-family: sans-serif; color:black ; font-weight:bold;">';
         $sql = mysqli_query($connect, "SELECT SUM(expense) as total FROM revenuexp");
           while  ($row3 = mysqli_fetch_array($sql)){
              $expenses = $row3['total'];                          
              }
              
                $sq2 = mysqli_query($connect, "SELECT SUM(revenue) as total FROM revenuexp");
...
//                        $total_savings = mysqli_query($connectect, "SELECT SUM(amount) as total FROM contribution WHERE name='$name'");      
//                          while  ($row = mysqli_fetch_array($total_savings)){
//                          $summation = $row['total'];                          
//                          }
//                            
//                            ////////////THIS IS TO SUM WITHDRAWAL
//                            $total_savings1 = mysqli_query($connectect, "SELECT SUM(amount) as total FROM witdraw WHERE name='$name'");      
