...
<head>
    <title>Daily Account Officer</title>
...
<body>
    ...
    <div id="pcoded" class="pcoded">
        ...
        <div class="pcoded-container navbar-wrapper">
                                ...
                                <div class="row align-items-center">
                                    <div class="col-md-8">
                                        <div class="page-header-title">
                                            <h5 class="m-b-10">DAILY ACCOUNT OFFICERS</h5>
         ...
         <form action="staff_all_acc_officer.php" method="POST" enctype="multipart/form-data" >
    ...
    <div class="input-group-append">
      <button class="btn btn-warning" id="btnsearch" name="btnsrch" type="submit">SEARCH</button>  
             ...
             <div class="row">                             
                 <div class="col-sm-2">
                                        <a href="staff_all_acc_officer.php" class="btn btn-dark">View All</a> 
                                     </div>
                 <div class="col-sm-2">
...
//     ====================STAFF AND NON PERFORMING IS SET=====================================================   
   ...
   echo ' <div class="card card-block table-border-style">';       
     ...
     <div ><CAPTION><h3 style="font-size:26px; color:black; font-weight:bold;" align="center">'.$staff_name.' ASSIGNED TO DAILY CUSTOMERS (NON-PERFORMING)</h3></CAPTION> </div> 
  ...
  <table  class="table table-bordered table-striped table-hover "  align="center">
   ...
   //    ###########################################################################################   
     ...
     $summation_repay11 = $summation_loan11 = 0;
                        $total_repay11 = mysqli_query($connect, "SELECT * FROM allloan WHERE indbalance > 1 AND remarks = 'loan disbursement' AND disburseloan > 0 AND acc_officer = '$staff_name'");      
                          while  ($row = mysqli_fetch_array($total_repay11)){
                          
                             
                                 $get_dailyenddate1 = $row['startdate'];
              ////CALCULATE CLOSING DATE
          $get_dailyenddate  = date('Y-m-d', strtotime($get_dailyenddate1. ' + 31 days'));
     ...
     @$pay_rate = ($repay_sum/$disburseloan) * 100;
        ...
        $days_remaining = floor($remaining / 86400);  
               ...
               if($days_remaining > 33 && $get_indbalance > 1 &&  $principal != $repay_sum ){
...
//     ------------------------------------------------------------------------             
       ...
       ////////////THIS IS TO SUM DAILY
                        $total_daily = mysqli_query($connect, "SELECT * FROM allloan WHERE indbalance > 1 AND remarks = 'loan disbursement' AND disburseloan > 0 AND acc_officer = '$staff_name'");      
...
//           $summation_loan = $summation_loan_d + $summation_loan_w + $summation_loan_m;
  ...
  //    ###########################################################################################    
        ...
        while  ($row = mysqli_fetch_array($result)){
                                    $get_dailyenddate1 = $row['startdate'];
              ////CALCULATE CLOSING DATE
          $get_dailyenddate  = date('Y-m-d', strtotime($get_dailyenddate1. ' + 32 days'));
             $get_indbalance = $row['indbalance'];
     $repay_sum = $row['repay_sum'];
     $disburseloan = $row['disburseloan'];
     $disburseloan_interest = $row['interest'];   
     ...
     @$pay_rate = ($repay_sum/$disburseloan) * 100;
              ///////////////////////////////////////
                 //GET NUMBER OF MONTHS & DAYS LEFT
                    $dated = strtotime("$get_dailyenddate1");
...
//           balance > 1 is to avoid repetition
   ...
   echo ' <div class="card card-block table-border-style">';       
     ...
     <div ><CAPTION><h3 style="font-size:26px; color:black; font-weight:bold;" align="center">'.$staff_name.' ASSIGNED TO DAILY CUSTOMERS</h3></CAPTION> </div> 
...
//   ^^^^^^^^^^^^^^^     FILTER NON PERFORMING IS SET AND STAFF IS NOT SET    ^^^^^^^^^^^^^^^^^^^^^
        ...
        $result = mysqli_query($connect, "SELECT * FROM allloan WHERE indbalance > 0 AND remarks = 'loan disbursement' AND    (reverse_alert = '' OR reverse_alert IS NULL) AND NOT acc_officer = '' ORDER BY id DESC");  
    
   echo ' <div class="card card-block table-border-style">';       
     echo     '<div class="table-responsive">

     <div ><CAPTION><h3 style="font-size:26px; color:black; font-weight:bold;" align="center">ALL ASSIGNED TO DAILY CUSTOMERS (NON-PERFORMING)</h3></CAPTION> </div> 
  ...
  <table  class="table table-bordered table-striped table-hover "  align="center">
   ...
   //    ###########################################################################################   
      ///GET ALL DEBTORS FOR DAILY
            ////////////THIS IS TO SUM DAILY
     $summation_repay11 = $summation_loan11 = 0;
                        $total_repay11 = mysqli_query($connect, "SELECT * FROM allloan WHERE indbalance > 1 AND remarks = 'loan disbursement' AND disburseloan > 0 AND NOT acc_officer = ''");      
                          while  ($row = mysqli_fetch_array($total_repay11)){
...
//     ------------------------------------------------------------------------             
       ...
       ////////////THIS IS TO SUM DAILY
                        $total_daily = mysqli_query($connect, "SELECT * FROM allloan WHERE indbalance > 1 AND remarks = 'loan disbursement' AND disburseloan > 0 AND NOT acc_officer = ''");      
                          while  ($row = mysqli_fetch_array($total_daily)){
...
//           balance > 1 is to avoid repetition
   ...
   echo ' <div class="card card-block table-border-style">';       
     ...
     <div ><CAPTION><h3 style="font-size:26px; color:black; font-weight:bold;" align="center">ALL ASSIGNED TO DAILY CUSTOMERS</h3></CAPTION> </div> 
  ...
  <table  class="table table-bordered table-striped table-hover "  align="center">
   ...
   //    ###########################################################################################   
      ///GET ALL DEBTORS FOR DAILY
            ////////////THIS IS TO SUM DAILY
                        $total_repay11 = mysqli_query($connect, "SELECT SUM(repay_sum) as total FROM allloan WHERE NOT acc_officer = ''");      
                          while  ($row = mysqli_fetch_array($total_repay11)){
                          $summation_repay11_d = $row['total'];                          
                          }
               
       ////////////THIS IS TO SUM DAILY
                        $total_daily = mysqli_query($connect, "SELECT SUM(disburseloan) as total FROM allloan WHERE NOT acc_officer = ''");      
                          while  ($row = mysqli_fetch_array($total_daily)){
                          $summation_loan11_d = $row['total'];                          
                          }
