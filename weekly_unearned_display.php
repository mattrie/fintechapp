...
//          $group = ""; 
     ...
     $result = mysqli_query($connect, "SELECT * FROM weekly_allloan WHERE indbalance > 0 AND remarks = 'loan disbursement'  AND name = '$codename_search'");
    $total_repay22 = mysqli_query($connect, "SELECT SUM(repay) as total FROM weekly_allloan WHERE name = '$codename_search'");      
                          while  ($row = mysqli_fetch_array($total_repay22)){
                          $summation_repay22 = $row['total'];                          
                          }
                $total_weekly = mysqli_query($connect, "SELECT SUM(disburseloan) as total, interest, SUM(interest) as sum_int2 FROM weekly_allloan WHERE name = '$codename_search'");      
           ...
           $summation_loan_weekly = $summation_loan_weekly - $summation_repay22;
             ...
             $sql_all_get112 = mysqli_query($connect, "SELECT * FROM weekly_allloan WHERE remarks = 'loan disbursement' AND name = '$codename_search'");
          while($row=mysqli_fetch_array($sql_all_get112)){
            $total_weekly = mysqli_query($connect, "SELECT SUM(disburseloan) as total, SUM(interest) as sum_int, SUM(repay_sum) as repay_sum FROM weekly_allloan WHERE name = '$codename_search'");      
                          while  ($row = mysqli_fetch_array($total_weekly)){
    ...
    echo ' <div class="card card-block table-border-style">';       
     ...
     echo '<CAPTION><h3 align="center" style="font-size:22px; color: black;">'.$codename_search.' CURRENT UNEARNED INCOME (WEEKLY)</h3></CAPTION>';
