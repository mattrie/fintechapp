...
<html>
    <head>
        <title>Month</title> <meta content="width=device-width, initial-scale=1.0" name="viewport">
        <meta charset="utf-8">
         <!-- Script -->
          ...
          $year = $_REQUEST['year']; 
            $inc=1;
             ...
             ////////////THIS IS TO SUM INTEREST PAID
                $total_payments = mysqli_query($connect, "SELECT SUM(monthinterset) as totali FROM investment WHERE month = '$month' AND year = '$year'");      
              while  ($row1 = mysqli_fetch_array($total_payments)){
             $summation_int_pay = $row1['totali'];                          
             }
            
//           balance > 1 is to avoid repetition
     $result = mysqli_query($connect, "SELECT * FROM investment WHERE month = '$month' AND year = '$year' ORDER BY id DESC");  
    
     echo ' <div class="card">';
         echo '<div class="card-block table-border-style">';
     
     echo     '<div class="table-responsive" >
     <CAPTION><h3 style="font-size:22px; color:black; font-weight:bold;" align="center">'.$month.' - '.$year.' INVESTORS || INTEREST TO PAY = N'.number_format($summation_int_pay).'</h3></CAPTION>
