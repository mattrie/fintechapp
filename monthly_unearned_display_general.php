...
include 'connection.php';
          ...
          //          $codename_search = $_REQUEST['codename']; 
               ...
               $result = mysqli_query($connect, "SELECT * FROM monthly_allloan WHERE   indbalance > 0 AND remarks = 'loan disbursement' ORDER BY id DESC");
               ...
               $total_repay22 = mysqli_query($connect, "SELECT SUM(repay) as total FROM monthly_allloan");      
                                    while  ($row = mysqli_fetch_array($total_repay22)){
                                    $summation_repay22 = $row['total'];                          
                                    }
                          $total_weekly = mysqli_query($connect, "SELECT SUM(disburseloan) as total, interest, SUM(interest) as sum_int2 FROM monthly_allloan");      
                     ...
                     $summation_loan_weekly = $summation_loan_weekly - $summation_repay22;
                       ...
                       $sql_all_get112 = mysqli_query($connect, "SELECT * FROM monthly_allloan WHERE remarks = 'loan disbursement'");
                    while($row=mysqli_fetch_array($sql_all_get112)){
                      $total_weekly = mysqli_query($connect, "SELECT SUM(disburseloan) as total, SUM(interest) as sum_int, SUM(repay_sum) as repay_sum FROM monthly_allloan");      
                                    while  ($row = mysqli_fetch_array($total_weekly)){
              ...
              echo ' <div class="card card-block table-border-style">';       
               ...
               echo '<CAPTION><h3 align="center" style="font-size:22px; color: black;">MONTHLY UNEARNED INCOME</h3></CAPTION>';
          ...
          //             echo '<th>Interest</th>';
                ...
                echo '<tbody class="" style="font-family: sans-serif; color:black ; font-weight:bold;">';
                      ...
                      echo "<td>------</td>";    
                       ...
                       echo "<td>TOTAL</td>"; 
                     echo "<td>" .number_format($weekly_unearned_income2)."</td>";
