...
//          $codename_search = $_REQUEST['codename']; 
     ...
     $result = mysqli_query($connect, "SELECT * FROM allloan WHERE   remarks = 'Bad Debt Settled by Finsolute'");
     $result1 = mysqli_query($connect, "SELECT * FROM weekly_allloan WHERE   remarks = 'Bad Debt Settled by Finsolute'");
     $result17 = mysqli_query($connect, "SELECT * FROM monthly_allloan WHERE   remarks = 'Bad Debt Settled by Finsolute'");
     
     echo ' <div class="card">';
         echo '<div class="card-block table-border-style">';
     
     echo     '<div class="table-responsive" >';             
     echo '<CAPTION><h3 align="center" style="font-size:22px; color: black;"><i>ALL</i> [BAD DEBT SETTLED BY FINSOLUTE]</h3></CAPTION>';
    ...
    ////////////THIS IS TO SUM DAILY
                        $total_daily= mysqli_query($connect, "SELECT SUM(repay) as total FROM allloan WHERE   remarks = 'Bad Debt Settled by Finsolute'");      
                          while  ($row = mysqli_fetch_array($total_daily)){
                          $summation_daily = $row['total'];                          
                          }
               
            ////////////THIS IS TO SUM weekly
                        $total_weekly= mysqli_query($connect, "SELECT SUM(repay) as total FROM weekly_allloan WHERE   remarks = 'Bad Debt Settled by Finsolute'");      
              ...
              ////////////THIS IS TO SUM monthly
                        $total_monthly= mysqli_query($connect, "SELECT SUM(repay) as total FROM monthly_allloan WHERE   remarks = 'Bad Debt Settled by Finsolute'");      
