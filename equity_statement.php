...
<head>
    <title>Weekly Equity Statement</title>
...
<body>
                                ...
                                <div class="row align-items-center">
                                    <div class="col-md-8">
                                        <div class="page-header-title">
                                            <h5 class="m-b-10">Weekly Equity Statement</h5>
   ...
   $debtor_info = mysqli_query($connect, "SELECT * FROM members WHERE namee = '$name'");        
        ...
        echo '<div class="row">';
         ...
         echo '<div class="col-sm-3" style="margin-left: 60px;">';
       ...
       echo '<input type="hidden" value=\''. $type .'\' id="type"/>';
           ...
           //////////////////////////////////----------------------------------////////////////////         
             //  *******************************************************************************************************                
                ...
                ///////THIS IS TO GET THE INDIVIDUAL equity BALANCE
                ///////////////THIS IS TO SUM ALL equity
            $sum_equity = mysqli_query($connect, "SELECT SUM(equity) as total FROM weekly_allloan WHERE name = '$name' ");      
              while  ($row = mysqli_fetch_assoc($sum_equity)){
         $summation_equity = $row['total']; 
//               $plus_interest = $row['interest'];  
              }
                 
                            
                ////////////THIS IS TO SUM ALLWITHDRAWALS
        $sum_withdrawequity = mysqli_query($connect, "SELECT SUM(withdrawequity) as total FROM weekly_allloan WHERE name = '$name'");      
         while  ($row1 = mysqli_fetch_assoc($sum_withdrawequity)){
         $summation_withdrawequity = $row1['total'];                          
         }
              
              $equity_balance =  $summation_equity - $summation_withdrawequity;
...
//            $_SESSION['av_bal']= $available_blance;FOR AJAX
            echo '<input type="hidden" value='. $equity_balance .' id="av_bal"/>'; 
        echo '<div class="col-sm-2">';
        echo "<label style='font-size: 18px; font-weight: bold;'>Equity Balance:<br> N ".number_format($equity_balance)."</label>";
        ...
        $sql_state = "SELECT * FROM weekly_allloan WHERE name = '$name' AND (equity > '1' OR withdrawequity > '1') ORDER BY id DESC";
            $result = mysqli_query($connect, $sql_state);
           
            
            echo '<div class="container">';
     echo  '<div class="table-responsive-sm">' ;
                 echo '<CAPTION><h3 align="center" style="font-size:20px; color: black;"> '.$name.' WEEKLY_EQUITY.</h3></CAPTION>';
