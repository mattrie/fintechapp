...
//                         $parent = $parent2 - $interest_to_revenue;
            ...
            if ($pay_pen == "P") {
                $result_revenue = mysqli_query($connect, "INSERT INTO revenuexp (remaks, category, type, revenue, date, date_format, month, year, collector, branch) VALUES('$name monthly Penalty', 'penalty_monthly', 'monthly', '$parent2', '$date', '$date_format', '$month','$year', 'admin', '$branch')");
...
//                $interest_reduce  =  $interest - $sum_interest_pay;
                ...
                if ($bad_debt != "ON") {
                    $result_revenue = mysqli_query($connect, "INSERT INTO revenuexp (remaks, category, type, revenue, date, date_format, month, year, collector, branch) VALUES('$name monthly interest', 'interest_monthly', 'monthly', '$real_interest', '$date', '$date_format', '$month','$year', 'admin', '$branch')");
                }
                ...
                swal({
                    ...
                    $bookmark_row = rand(10, 100000000);
                    $sql_statement = "INSERT INTO monthly_allloan (name, type, remarks, repay, interest, codename, date, date_format, month, year, district, collector, timestamp, insurance_out, date_posted, bookmark_row, branch) VALUES ('$name', '$loan_type', '$remarks', '$parent2', '$real_interest', '$telephone', '$date' , '$date_format', '$month', '$year', '$regnum', 'admin', '$time_stamp1', '$insurance_out_now', '$date_posted', '$bookmark_row', '$branch')";
                    $result = mysqli_query($connect, $sql_statement);
...
//                $interest_reduce  =  $interest - $sum_interest_pay;
                    ...
                    ///////THIS IS TO GET THE INDIVIDUAL BALANCE
                    ////////////THIS IS TO SUM LOAN
                    $code_loan = mysqli_query($connect, "SELECT SUM(disburseloan) as total FROM monthly_allloan WHERE codename = '$telephone' AND (disburseloan > '1' OR disburseloan < -1)");
                    while ($row = mysqli_fetch_assoc($code_loan)) {
                        $summation_loan2 = $row['total'];
                        //                           $plus_interest = $row['interest'];  
                    }
