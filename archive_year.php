...
//          $codename_search = $_REQUEST['codename']; 
       ...
       $result = mysqli_query($connect, "SELECT * FROM allloan WHERE  year = '$year' AND (indbalance < 1) AND  (reverse_alert = '' OR reverse_alert IS NULL) AND remarks = 'loan disbursement' ORDER BY id DESC");
     $result1 = mysqli_query($connect, "SELECT * FROM weekly_allloan WHERE  year = '$year' AND (indbalance < 1) AND  (reverse_alert = '' OR reverse_alert IS NULL) AND remarks = 'loan disbursement' ORDER BY id DESC");
     $result17 = mysqli_query($connect, "SELECT * FROM monthly_allloan WHERE  year = '$year' AND (indbalance < 1) AND  (reverse_alert = '' OR reverse_alert IS NULL) AND remarks = 'loan disbursement' ORDER BY id DESC");
     
   echo ' <div class="card card-block table-border-style">';       
     echo     '<div class="table-responsive">';           
     echo '<CAPTION><h3 align="center" style="font-size:22px; color: black;"><i>'.$year.'</i> [FINISHED TRANSACTION(S)]</h3></CAPTION>';
