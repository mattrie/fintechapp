...
//          $codename = $_REQUEST['codename']; 
     ...
     //////////////////////FOR archive_display_general.php
    $result = mysqli_query($connect, "SELECT * FROM allloan WHERE  (indbalance < 1) AND  (reverse_alert = '' OR reverse_alert IS NULL) AND remarks = 'loan disbursement' ORDER BY id DESC");
     $result1 = mysqli_query($connect, "SELECT * FROM weekly_allloan WHERE  (indbalance < 1) AND  (reverse_alert = '' OR reverse_alert IS NULL) AND remarks = 'loan disbursement' ORDER BY id DESC");
     $result17 = mysqli_query($connect, "SELECT * FROM monthly_allloan WHERE  (indbalance < 1) AND  (reverse_alert = '' OR reverse_alert IS NULL) AND remarks = 'loan disbursement' ORDER BY id DESC");
      
     echo ' <div class="card">';
         echo '<div class="card-block table-border-style">';
     
     echo     '<div class="table-responsive">';      
     echo '<CAPTION><h3 align="center" style="font-size:22px; color: black;"><i>ALL</i> [FINISHED TRANSACTION(S)]</h3></CAPTION>';
